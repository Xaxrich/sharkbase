...
## 我的评注
- **赞同**："克制式"设计哲学是这个视频最有价值的思想。52个工具只暴露~20个、默认拒绝所有权限、五种协作模式只开放两种——这和"做加法"的竞品形成鲜明对比。底层逻辑是：模型上下文中信息越少决策质量越高，这在 [[RLVR训练范式]] 中被同一逻辑佐证（环境约束精准度 > 数据量）。
- **质疑**：小韩对 Claude Code 有明显的偏好，缺少对"克制式"设计局限的批判。比如：三层工具过滤意味着很多实验性功能被隐藏，用户无法按需开启；权限默认拒绝虽然安全，但频繁的授权弹窗对用户体验是真实损耗——"安全的代价"没有被充分讨论。
- **关联**：Claude Code 的"克制"和 [[MVP思维]] 的"做减法"是同一个元模式在不同领域的体现——详见 [[驾驭工程]] 的深层联系。Auto Dream 机制（每24小时清理记忆、去重、解决冲突）是 Garbage Collection 思想在 Agent 领域的落地，和代码库需要定期重构、产品需要定期砍功能是同一个道理。
- **待深入**：协调者模式和 Team 模式开放后会怎样改变 Agent 协作范式？ForkAgent 利用服务端 Token 缓存的策略在第三方 API 代理（无缓存）下完全失效，这对企业用户意味着什么？

## 与其他知识的关联
- 相关概念：[[驾驭工程]]、[[MCP Server]]
- 相关实体：[[小韩]]
- 相关主题：[[LLM-Agent工程实践]]
