...
Reads config/local.yaml and exposes paths and settings as a dict.
...
def get_path(key: str, default: str = "") -> Path:
    """Get a path from config, returning Path object."""
    cfg = load_config()
    # Try nested: workspace.bili_dir → cfg["workspace"]["bili_dir"]
    parts = key.split(".")
    val = cfg
    for p in parts:
...
# Convenience: project root derived from config or script location
